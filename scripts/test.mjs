...
execFileSync('python3',['-m','py_compile','scripts/import-products.py']);execFileSync('python3',['scripts/test-import-products.py'],{stdio:'inherit'});
const cfg=JSON.parse(fs.readFileSync(path.join(dist,'admin/cms/config.yml')));assert.equal(cfg.backend.repo,'ArtanSulejmani/artink-studio-site');assert.equal(cfg.collections[0].name,'products');assert.equal(cfg.collections[0].create,true);assert.equal(cfg.collections[0].delete,true);assert(cfg.collections.some(c=>c.name==='categories'));assert(cfg.collections.some(c=>c.name==='imports'));assert.equal(cfg.collections.at(-1).name,'release');
assert(fs.existsSync(path.join(dist,'downloads/artink-products-import-template.xlsx')));assert(fs.readFileSync(path.join(dist,'admin/index.html'),'utf8').includes('Download Excel template'));
